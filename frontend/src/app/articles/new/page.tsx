...
function getAparatId(url: string): string | null {
  const m = url.match(/aparat\.com\/v\/([a-zA-Z0-9_-]+)/);
  return m ? m[1] : null;
}
...
export default function SubmitArticlePage() {
  ...
  const addGalleryImage = () => {
    if (galleryUrl) { setGallery([...gallery, { url: galleryUrl }]); setGalleryUrl(''); }
  };
  ...
  const addVideo = () => {
    if (!videoUrl) return;
    const vid: VideoEmbed = { url: videoUrl, title: videoTitle || undefined, source: videoSource };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.message?.[0] || data.message || 'خطا در ارسال مقاله');
      }
      setSuccess(true);
      window.scrollTo(0, 0);
    } catch (err: any) {
      setError(err.message || 'خطا در ارسال مقاله');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (success) {
    return (
      <section className="max-w-2xl mx-auto px-4 py-24 text-center">
        ...
        <h1 className="text-3xl font-black text-white mb-4">مقاله شما با موفقیت ارسال شد!</h1>
        <p className="text-gray-custom mb-8">
          مقاله شما پس از بررسی توسط تیم مدیریت منتشر خواهد شد.
        </p>
        <button
          onClick={() => router.push('/')}
  ...
  return (
    <section className="max-w-4xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-black text-white mb-2">ارسال مقاله جدید</h1>
      <p className="text-gray-custom mb-8">مقاله شما پس از تایید مدیریت در سایت منتشر خواهد شد.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {gallery.map((img, i) => (
                <div key={i} className="relative group">
                  ...
                  <button type="button" onClick={() => setGallery(gallery.filter((_, j) => j !== i))}
                    className="absolute top-1 right-1 bg-red-600 text-white w-5 h-5 rounded-full text-xs opacity-0 group-hover:opacity-100">×</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-dark-light border border-white/10 rounded-xl p-6 space-y-4">
          ...
          <div className="flex gap-2">
            <select value={videoSource} onChange={e => setVideoSource(e.target.value as any)}
              className="px-3 py-3 bg-dark border border-white/10 rounded-lg text-white text-sm">
              <option value="youtube">یوتیوب</option>
              <option value="aparat">آپارات</option>
              <option value="upload">آپلودی</option>
            </select>
          ...
          {videos.length > 0 && (
            <div className="space-y-2">
              {videos.map((v, i) => (
                <div key={i} className="flex justify-between items-center bg-dark rounded-lg px-4 py-2">
                  <span className="text-sm text-gray-custom truncate">{v.title || v.url}</span>
                  <button type="button" onClick={() => setVideos(videos.filter((_, j) => j !== i))} className="text-red-400 text-sm">حذف</button>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Attachments */}
        <div className="bg-dark-light border border-white/10 rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold text-white">پیوست‌ها (فایل)</h2>
          <label className="inline-block px-4 py-2 bg-dark border border-white/10 rounded-lg text-gray-custom cursor-pointer hover:text-white text-sm">
            آپلود فایل (PDF, ZIP, ...)
          ...
          {attachments.length > 0 && (
            <div className="space-y-2">
              {attachments.map((att, i) => (
                <div key={i} className="flex justify-between items-center bg-dark rounded-lg px-4 py-2">
                  <span className="text-sm text-gray-custom">{att.filename}</span>
                  <button type="button" onClick={() => setAttachments(attachments.filter((_, j) => j !== i))} className="text-red-400 text-sm">حذف</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-dark-light border border-white/10 rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-custom mb-1">دسته‌بندی‌ها</label>
                ...
                {categories.map(cat => (
                  <label key={cat._id} className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="checkbox" checked={selectedCategories.includes(cat._id)}
                      ...
                      className="rounded border-gray-500 bg-dark" />
                    {cat.name}
                  </label>
                ))}
                {categories.length === 0 && <p className="text-gray-500 text-sm">دسته‌بندی‌ای یافت نشد</p>}
              </div>
            </div>
            <div>
        ...
        <button type="submit" disabled={saving}
          className="w-full py-4 gradient-primary text-white rounded-xl font-bold hover:opacity-90 transition-opacity disabled:opacity-50 text-lg">
          {saving ? 'در حال ارسال...' : 'ارسال مقاله برای بررسی'}
        </button>
      </form>
    </section>
  );
