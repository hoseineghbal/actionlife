...
export default function MyProductsPage() {
  ...
  const handleStopSale = async (id: string) => {
    if (!confirm("آیا از توقف فروش این محصول اطمینان دارید؟ محصول از حالت انتشار خارج می‌شود.")) return;
    const token = localStorage.getItem("access_token")!;
    try {
      await updateStoreProduct(token, id, { status: "archived" });
      setProducts((prev) =>
        prev.map((p) => (p._id === id ? { ...p, status: "archived" } as StoreProduct : p)),
      );
    } catch (err: any) {
      alert(err.message ?? "خطا در توقف فروش");
    }
  };
  ...
  const handleRemoveDiscount = async (productId: string, index: number) => {
    ...
    const token = localStorage.getItem("access_token")!;
    try {
      const updated = await setProductDiscounts(token, productId, updatedDiscounts);
      setProducts((prev) =>
        prev.map((p) => (p._id === productId ? { ...p, ...updated } : p)),
      );
    } catch (err: any) {
              ...
              <div className="space-y-4">
                {products.map((product) => (
                  ...
                  >
                    <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-sm text-gray-custom">
                          {editingPrice === product._id ? (
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                type="number"
                                placeholder="تخفیف ساده"
                                value={newDiscount}
                                onChange={(e) => setNewDiscount(e.target.value)}
                                className="w-28 px-3 py-1.5 bg-dark border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-accent/50"
                              />
                              <button
                          ...
                          {product.discounts && product.discounts.length > 0 && (() => {
                            const now = new Date();
                            const active = product.discounts.some(
                              (d) => now >= new Date(d.startDate) && now <= new Date(d.endDate)
                            );
                            return active ? (
                              <span className="px-2 py-0.5 bg-green-500/10 text-green-400 rounded text-xs">تخفیف فعال</span>
                            ) : null;
                          })()}
                        </div>
                      </div>
                      ...
                      <div className="flex md:flex-col items-center justify-center gap-2 shrink-0">
                        {product.status !== "published" ? (
                          ...
                          <>
                            <button
                              onClick={() => handleStopSale(product._id)}
                              className="w-full px-4 py-2 bg-orange-500/10 text-orange-400 hover:bg-orange-500/20 rounded-lg text-xs font-medium transition-colors"
                            >
                              توقف فروش
                            </button>
                            <button
                              onClick={() =>
                                setExpandedDiscount(
                                  expandedDiscount === product._id ? null : product._id
                                )
                              }
                              className="w-full px-4 py-2 bg-accent/10 text-accent hover:bg-accent/20 rounded-lg text-xs font-medium transition-colors"
                            >
                              {expandedDiscount === product._id ? "بستن تخفیف" : "مدیریت تخفیف"}
                            </button>
                          </>
                        )}
                      </div>
                    ...
                    {expandedDiscount === product._id && (
                      <div className="mt-4 pt-4 border-t border-white/5">
                        <h4 className="text-sm font-medium text-white mb-3">مدیریت تخفیف‌های زمان‌دار</h4>

                        {/* Existing Discounts */}
                        {(product.discounts && product.discounts.length > 0) && (
                          <div className="mb-4 space-y-2">
                            <p className="text-xs text-gray-custom">تخفیف‌های ثبت شده:</p>
                                ...
                                >
                                  <div>
                                    <span className="font-bold text-accent">
                                      {formatPrice(d.discountPrice)} توکن
                                    </span>
                                    <span className="text-gray-custom mr-3 text-xs">
                                      {formatJalaliDateTime(d.startDate)} تا {formatJalaliDateTime(d.endDate)}
                                    </span>
                                    {isActive && (
                                      <span className="mr-2 text-xs bg-green-500/10 text-green-400 px-2 py-0.5 rounded">
                                        فعال
                                      </span>
                                    )}
                                  </div>
                                  <button
                                    onClick={() => handleRemoveDiscount(product._id, i)}
                        ...
                        {/* Add New Discount */}
                        <div className="p-4 bg-dark border border-white/10 rounded-xl space-y-3">
                          <p className="text-xs font-medium text-gray-custom mb-2">افزودن تخفیف جدید</p>

                          <div>
                            <label className="block text-xs text-gray-custom mb-1">
                              قیمت با تخفیف (توکن)
                            </label>
                            <input
                              type="number"
                              ...
                              placeholder={`کمتر از ${formatPrice(product.price)}`}
